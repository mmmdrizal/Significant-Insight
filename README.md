...
Berdasarkan hasil evaluasi, kombinasi LightGBM dengan penanganan outlier menggunakan IQR dan tanpa penanganan imbalance terbukti memberikan performa terbaik, dengan F1 Score tertinggi sebesar 0.9838. Model ini tidak hanya unggul dalam akurasi, tetapi juga dalam konsistensi prediksi. Salah satu alasan utama performa tinggi LightGBM adalah kemampuannya dalam memproses data numerik dalam jumlah besar secara efisien melalui pendekatan histogram-based decision tree, yang sangat sesuai dengan karakteristik dataset ini yang memiliki banyak fitur numerik dan tidak terlalu banyak missing value. Selain itu, struktur data yang telah dibersihkan menggunakan IQR membantu LightGBM untuk fokus pada pola-pola dominan tanpa terganggu oleh outlier ekstrem.

Meskipun XGBoost juga menunjukkan performa kompetitif, performanya sedikit rendah dalam beberapa kombinasi, kemungkinan karena kompleksitas proses boosting-nya yang lebih tinggi dan kebutuhan tuning parameter yang lebih sensitif terhadap variasi distribusi data. Di sisi lain, Random Forest menghasilkan performa yang baik namun cenderung lebih stabil (tidak sebaik boosting models) karena bersifat bagging-based, yang umumnya kurang responsif terhadap pola minoritas dalam data imbalanced, bahkan dengan bantuan SMOTE.

Model terbaik (LGBM + IQR + No Handling) ini berhasil menghadirkan enam fitur paling penting yang berpengaruh terhadap ketahanan pangan, yaitu: NCPR, Kemiskinan (%), Tanpa Air Bersih (%), Angka Harapan Hidup (tahun), Jumlah Penduduk (rasio), dan Tanpa Listrik (%).
